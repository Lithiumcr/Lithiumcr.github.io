...
论文后面的形式化证明，则是在维护几个朴素但关键的不变量，证明这些机制组合起来依然奏效。**Preservation** 说系统原本良构，走一步仍然良构。**Temporal composability** 说组件卸载后不留垃圾。**Spatial composability** 说活跃组件凡有依赖，必被满足。**Progress** 说系统不会无故卡死，有路可走就能继续前进。**Confluence** 说互不相关的操作，不能因调度顺序不同而分道扬镳，动态历史最终容不下幽灵的痕迹。这些话听起来像废话，但无数工程事故往往就出在这些废话上。
...
再横向看，**范畴论进入工程也并非第一次**。成功者如 Haskell，把副作用变成可组合的值；Scala 的 Cats Effect、ZIO 把这套思路做成生产级并发运行时；Kestrel 的 Specware 系统更用范畴论做软件规格说明、合成、维护，证明其在工业级开发中的可行性。当然反例也不少，早期 FRP、过度 Monad Transformer、过度 tagless-final，论文里应有尽有，落地后败给调试、性能、招聘、交付压力等各种问题。
