...
- [ ] GitHub Actions auto-builds on push to `master` after cutover merge
- [ ] GitHub Pages deployment verified from remote
...
- [x] Keep `/Users/administrator/work/Lithiumcr.github.io/.git` and switch the working tree to Hugo source on `hugo-cutover-2026-06`
...
- [ ] Commit and push `hugo-cutover-2026-06` for review / PR
- [ ] Merge cutover branch into `master`
...
| 2: New site skeleton | Core done | Theme, layouts, config, local build, and master-based Actions workflow are in place; deployed GitHub Pages verification pending |
| 3: Content model | Done | Sections, taxonomies, archetypes, taxonomy templates complete |
| 4: Content migration | Substantially done | 83 aliases generated, 55 essays + 30 archives present; deployment URL smoke test pending |
| 5: Search & subscription | Locally working | RSS builds locally; `/search.json` is generated by Hugo and consumed by the `/search/` page |
...
| 7: Citability | Core done | Anchors, citation block, policy page complete; archive statement and deployed rendering check pending |
| Release readiness | In cutover | Hugo source is staged in `Lithiumcr.github.io` on `hugo-cutover-2026-06`; push, merge to `master`, Pages Actions setting, and online smoke test remain |
